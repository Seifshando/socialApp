...
import style from './Register.jsx'
...
export default function Register() {
  ...
  const schema = z.object({
    name : z.string().min(1, "must be one character atLeast").max(10, 'max length is 10 characters'),
    ...
    dateOfBirth : z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'invalid date').refine((date) => {
      ...
      return customDate < nowDate ;

    } , "can't use future date !"),

    gender : z.enum(['male', 'female'] , 'gender is required'),
  }).refine((obj) =>  obj.rePassword === obj.password , {
...
<>
<form onSubmit={handleSubmit(handleRegister)} className="max-w-md mx-auto">

    {apierror && <h1 className='text-center bg-red-700 p-1 m-5 text-white'>{apierror}</h1>}

  <div className="relative z-0 w-full mb-5 group">
      <input type="name" {...register("name")} id="name" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "  />
      ...
      {formState.errors.name && formState.touchedFields.name ? <p className='text-red-500 text-center font-semibold'>{formState.errors.name.message}</p> : ""}
  </div>
  <div className="relative z-0 w-full mb-5 group">
      ...
      {formState.errors.email && formState.touchedFields.email ? <p className='text-red-500 text-center font-semibold'>{formState.errors.email.message}</p> : ""}
  </div>
  <div className="relative z-0 w-full mb-5 group">
      ...
      {formState.errors.password && formState.touchedFields.password ? <p className='text-red-500 text-center font-semibold'>{formState.errors.password.message}</p> : ""}
  </div>
  <div className="relative z-0 w-full mb-5 group">
      ...
      {formState.errors.rePassword && formState.touchedFields.rePassword ? <p className='text-red-500 text-center font-semibold'>{formState.errors.rePassword.message}</p> : ""}
  </div>
  <div className="relative z-0 w-full mb-5 group">
      ...
      <label htmlFor="dateOfBirth" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Enter your dateOfBirth...</label>
      {formState.errors.dateOfBirth && formState.touchedFields.dateOfBirth ? <p className='text-red-500 text-center font-semibold'>{formState.errors.dateOfBirth.message}</p> : ""}
  </div>
  ...
  <div className='flex gap-4'>
    <div className="flex items-center">
      <input id="gender" type="radio" value="male" {...register("gender")} className="w-4 h-4 mb-3 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
      <label htmlFor="gender" className="ms-2 mb-3 text-sm font-medium text-gray-900 dark:text-gray-300">male</label>
    </div>

    <div className="flex items-center">
      <input   id="gender2" type="radio" value="female" {...register("gender")} className="w-4 h-4 mb-3 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
      <label htmlFor="gender2" className="ms-2 mb-3 text-sm font-medium text-gray-900 dark:text-gray-300">female</label>
    </div>
    {formState.errors.gender && formState.touchedFields.gender ? <p className='text-red-500 text-center font-semibold'>{formState.errors.gender.message}</p>: ""}
  </div>
